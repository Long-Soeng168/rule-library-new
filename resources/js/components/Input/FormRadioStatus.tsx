...
const FormRadioStatus = ({ name, label, required, options, value, onChange, error }: FormRadioStatusProps) => {
    return (
        <div className="grid content-start gap-2">
            {label && <FormLabel label={label} required={required} />}

            <RadioGroupPrimitive.Root name={name} value={value} onValueChange={onChange} className="grid w-full max-w-md grid-cols-3 gap-4">
                {options.map((option) => (
                    ...
                    >
                        ...
                        <span className="text-foreground tracking-tight">{option.label}</span>
                        {option.description && <p className="text-xs pt-1 text-muted-foreground">{option.description}</p>}
                    </RadioGroupPrimitive.Item>
                ))}
