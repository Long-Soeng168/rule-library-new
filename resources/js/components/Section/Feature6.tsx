        ...
        <div className="mt-10">
            <div className="grid grid-cols-2 justify-center gap-x-10 gap-y-16 lg:grid-cols-4">
                {statsData?.map((stat: any, idx: number) => {
                    ...
                    return (
                        <div key={stat.id}>
                            <span className="text-5xl font-bold text-primary md:text-6xl">{stat.title}</span>

                            {stat.description && <p className="mt-4 text-[17px] text-muted-foreground">{stat.description}</p>}
                        </div>
                    );
                })}
            </div>
