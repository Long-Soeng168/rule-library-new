...
import AlertStatic from '@/components/Alert/AlertStatic';
...
export default function Create({ editData, readOnly }: { editData?: any; readOnly?: boolean }) {
    ...
    const [flashMessage, setFlashMessage] = useState<{ message: string; type: string }>({
        ...
        type: flash?.success ? 'success' : 'message',
    });
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'Manage Library Data', href: '/manage-library-data' },
    ...
    // map your status to alert types
    const statusTypeMap: Record<string, 'success' | 'info' | 'error'> = {
        pending: 'info',
        approved: 'success',
        rejected: 'error',
    };

    // get type based on editData.status
    const alertType = statusTypeMap[editData?.status || 'pending'] || 'info';
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <form onSubmit={onSubmit} className="form">
                ...
                <AlertStatic type={alertType} message={toCapitalize(editData?.status) || 'Pending'} />
                <div>
                        ...
                        <div className="flex flex-wrap justify-between gap-2">
                            <div>
                                <h1 className="text-start text-2xl font-bold">
                                    {t('Library Data')} <span className="text-primary">({data.year_of_data})</span>
                                </h1>
                                <div className="flex items-center">
                                    {t('Last Updated')} : <TableCellDate className="p-0 pl-1" value={`${editData?.updated_at}`} />
                                </div>
                            </div>
                            {editData?.id && (
                                <Link href={`/libraries/${editData.id}`}>
                                    <Button variant="outline" className="rounded">
                                        {t('View Library')} <ArrowRight />
                                    </Button>
                                </Link>
                            )}
                        </div>

                        <h2 className="mt-6 mb-4 text-xl font-semibold">{t('I. General Information')}</h2>
