            ...
            <div className="section-container">
                ...
                <header className="my-8 text-center">
                    ...
                    <p className="mt-2 text-muted-foreground">
                        ស្វែងរក ឬមើលបញ្ជីបុគ្គលិកតាមផ្នែក និងវេន
                    </p>
                </header>
                ...
                <div className="mb-8 flex flex-col gap-4 bg-background/80 py-4 backdrop-blur-sm sm:flex-row">
                    <input
                        type="text"
                        placeholder="វាយឈ្មោះ, ភារកិច្ច, ឬបណ្ណាល័យ..."
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                        className="flex-grow rounded-lg border border-border p-3 text-foreground placeholder:text-muted-foreground transition focus:ring-2 focus:ring-primary focus:outline-none"
                    />
                </div>

                {filteredStaff.length === 0 ? (
                    <div className="py-10 text-center text-lg text-muted-foreground">
                        មិនមានលទ្ធផលសម្រាប់ការស្វែងរករបស់អ្នកទេ។
                    </div>
                ) : (
                    libraryOrder.map((libraryName) => {
                        const libraryStaff = filteredStaff.filter(
                            (s) => s.library === libraryName
                        );
                        ...
                        return (
                            <section
                                key={libraryName}
                                className="mb-8 rounded-lg border border-border bg-muted p-4 sm:p-6"
                            >
                                ...
                                {Object.entries(shifts).map(
                                    ([shiftName, staffArr]) =>
                                        staffArr.length > 0 && (
                                            <div key={shiftName}>
                                                <h3 className="mt-5 mb-3 text-xl font-semibold text-foreground">
                                                    {shiftName}
                                                </h3>
                                                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
                                                    {staffArr.map((staff, i) => (
                                                        <div
                                                            key={i}
                                                            className="flex h-full flex-col rounded-lg border border-border bg-background dark:bg-white/8 p-5 shadow transition hover:shadow-md"
                                                        >
